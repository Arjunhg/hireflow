...
const CTADialogBox = ({
  ...
  const handleClick = async () => {
    setLoading(true)
    console.log('CTADialogBox handleClick called with userId:', userId, 'webinar:', webinar.id);
    try {
      if (webinar.ctaType === 'BOOK_A_CALL') {
        const redirectUrl = `/live-webinar/${webinar.id}/call?attendeeId=${userId}`;
        console.log('Redirecting to:', redirectUrl);
        router.push(redirectUrl)
      } else {
        // No Stripe: just show a toast and close dialog
